    ...
    <div className="space-y-6">
      <section className="rounded-[32px] border border-white/10 bg-gradient-to-br from-brand/20 via-slate-900 to-slate-950 p-6">
        ...
        <p className="mt-3 max-w-2xl text-slate-300">
          Plataforma pronta para vender sem taxa de marketplace, com checkout
          por WhatsApp e base preparada para assinaturas SaaS.
        </p>
        <Link
          to={`/${restaurant?.slug ?? ""}`}
      ...
      {onboarding ? (
        <OnboardingChecklist
          data={onboarding}
          onPrimaryAction={() => navigate("/admin/products")}
        />
      ) : null}
          ...
          <>
            ...
            <MetricCard
              ...
              value={formatCurrency(
                finance?.revenue || insights?.summary.totalRevenue || 0,
              )}
              helper={`Mes: ${formatCurrency(
                insights?.summary.monthlyRevenue || 0,
              )}`}
            />
            <MetricCard
              label="Ticket medio"
              value={formatCurrency(
                finance?.averageTicket || insights?.summary.averageTicket || 0,
              )}
              helper={`Conversao estimada: ${
                insights?.summary.conversionRate || 0
              }%`}
            />
            <MetricCard
              label="Lucro estimado"
              value={formatCurrency(finance?.estimatedProfit || 0)}
              helper={`Economia vs marketplace: ${formatCurrency(
                insights?.summary.savedFees || 0,
              )}`}
            />
          </>
        )}
      ...
      <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[28px] border border-white/10 bg-white/5 p-5">
          <SectionHeading
            ...
            subtitle="Ranking para orientar promos e destaque no cardapio."
          />
          <div className="mt-5 space-y-3">
            {insights?.topProducts && insights.topProducts.length > 0 ? (
              ...
              <p className="mt-4 text-sm text-slate-400">
                Os dados aparecerao apos os primeiros pedidos.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-[28px] border border-white/10 bg-white/5 p-5">
            <SectionHeading
              ...
              subtitle="Margem estimada para orientar mix e precificacao."
            />
            <div className="mt-4 space-y-3">
              ...
              {!(insights?.topProfitableProducts.length ?? 0) ? (
                <p className="text-sm text-slate-400">
                  As margens aparecem conforme os custos entram no catalogo.
                </p>
              ) : null}
            </div>
          </div>
          ...
          <div className="rounded-[28px] border border-white/10 bg-white/5 p-5">
            <SectionHeading
              ...
              subtitle="Templates fake, mas criveis para demos comerciais."
            />
            <div className="mt-4 space-y-3">
              ...
              {templates.length === 0 ? (
                <p className="text-sm text-slate-400">
                  Os templates sao opcionais e podem ser carregados depois.
                </p>
              ) : null}
            </div>
          </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <div className="rounded-[28px] border border-white/10 bg-white/5 p-5">
          <SectionHeading
            ...
            subtitle="Base para escala e campanhas com maior conversao."
          />
          <div className="mt-4 space-y-3">
              ...
              <div key={item.hour}>
                ...
                <div className="h-2 overflow-hidden rounded-full bg-white/10">
                  <div
                    ...
                    style={{ width: `${Math.min(100, item.orders * 18)}%` }}
                  />
                </div>
              </div>
            ))}
        ...
        <div className="rounded-[28px] border border-white/10 bg-white/5 p-5">
          <SectionHeading
            ...
            subtitle="Aceleradores de ticket medio e recorrencia."
          />
          <div className="mt-4 space-y-3">
      ...
      <section className="rounded-[28px] border border-emerald-500/20 bg-emerald-500/10 p-5">
        <SectionHeading
          title="Plano atual"
          subtitle="Modelo de monetizacao pronto para venda B2B."
        />
        <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-3xl font-black text-white">
              {restaurant?.plan ?? "Plano não definido"}
            </p>
            <p className="mt-2 text-sm text-slate-300">
              Canal principal: WhatsApp direto, sem taxa por pedido.
            </p>
          </div>
          <Link
            to="/admin/billing"

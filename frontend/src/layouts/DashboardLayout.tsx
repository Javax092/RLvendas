import { BarChart3, CreditCard, LogOut, Megaphone, Package, Sandwich, Settings, Users } from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="mx-auto flex max-w-7xl gap-6 px-4 py-6 lg:px-8">
        <aside className="hidden w-72 shrink-0 rounded-[28px] border border-white/10 bg-white/5 p-5 lg:block">
          <Link to={`/${session?.restaurant.slug}`} className="flex items-center gap-3 text-lg font-bold">
          ...
          <button onClick={signOut} className="mt-8 flex items-center gap-2 text-sm text-slate-400">
            <LogOut size={16} />
            Sair
          </button>
        </aside>
